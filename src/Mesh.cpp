...
void Mesh::_initialize_mesh() {
   ...
   //Attributes f�r Vertex Strukten; Position, Normal och TexturKoordinat
   //Position
...
void Mesh::_draw(ShaderProgram &shaderProgram) {
   ...
   for (GLuint i = 0; i < this->texture_list.size(); i++) {
      ...
      //st�lla in vilken Textur vi jobbar p�, vi kan h�gst ha 32 (eller 16?)
   ...
   //Bind OpenGL till standard v�rderna...
   glBindVertexArray(0);
   for (GLuint i = 0; i < this->texture_list.size(); i++) {

...
void main() {
   ...
   switch ( render_mode ) {
      ...
      case mode_textureless:
         lighting = vec3(0); // start off with ambient light
         ...
         if ( light.type == point_light_t ) { // TODO: take one array of each light type and have a loop for each instead
            ...
            if ( distance < radius ) {
               ...
               vec3  diffuse_impact   = vec3(0.2) * light.rgb * (light_modulation * light.intensity * quad_falloff);
      ...
      lighting += emit_rgb;
      break; // end of mode_textureless case

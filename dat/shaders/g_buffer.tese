...
// alternatives: fractional_odd_spacing, fractional_even_spacing
...
void main()
{
	...
	// set out matrix to new matrix with interpolated vectors
	tbn_gs = mat3(tangent, bitangent, normal);

	// sample displacement map based on interpolated uv_coords
	vec4 displacement = texture( tex_disp, uv_gs );

	// Apply displacement on interpolated position
	vec3 halfvec = vec3(.5f,.5f,.5f); // halfvec makes our displacement value go up/down
